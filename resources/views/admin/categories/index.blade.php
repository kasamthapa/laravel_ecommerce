<x-layouts.admin title="Categories - Luma Lens Admin">
    <x-admin.page-header title="Categories" :subtitle="$categories->total().' categories.'">
        <x-slot:actions>
            <a href="{{ route('admin.categories.create') }}" class="motion-press inline-flex items-center gap-2 rounded-full bg-accent px-5 py-3 text-sm font-black text-white hover:bg-ink">
                ...
                <tbody class="divide-y divide-cream-dim">
                    @forelse ($categories as $category)
                        <tr class="transition hover:bg-cream-dim">
                            <td class="px-6 py-3.5 font-bold text-ink">{{ $category->name }}</td>
                            <td class="px-6 py-3.5 text-stone">{{ $category->products_count }}</td>
                            <td class="px-6 py-3.5">
                                <div class="flex items-center justify-end gap-1">
                                    <a href="{{ route('admin.categories.edit', $category) }}" aria-label="Edit {{ $category->name }}" class="grid h-9 w-9 place-items-center rounded-lg text-stone hover:bg-accent-tint hover:text-accent">
                                    ...
                                    <form method="POST" action="{{ route('admin.categories.destroy', $category) }}" onsubmit="return confirm('Delete {{ $category->name }}?');">
                                        @csrf
                                        @method('DELETE')
                                        <button aria-label="Delete {{ $category->name }}" class="grid h-9 w-9 place-items-center rounded-lg text-stone hover:bg-error-tint hover:text-error">
                                            <svg class="h-[1.125rem] w-[1.125rem]" viewBox="0 0 24 24" fill="none" aria-hidden="true">
                    ...
                    @empty
                        <tr>
                            <td colspan="3" class="px-6 py-10 text-center text-stone">No categories yet.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
