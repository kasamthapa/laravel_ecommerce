<x-layouts.storefront title="Checkout - Luma Lens" :cart-count="$cartCount">
    <section class="mx-auto max-w-[75rem] px-4 py-10 sm:px-8 lg:py-14">
        <p class="text-xs font-medium uppercase tracking-[0.14em] text-smoke">Checkout</p>
        ...
        <ol class="mt-8 flex items-center gap-3 text-xs font-medium uppercase tracking-wide text-smoke" aria-label="Checkout progress">
            <li class="flex items-center gap-2 text-bone"><span class="grid h-6 w-6 place-items-center rounded-full bg-volt text-bone">1</span> Contact</li>
            <li class="h-px w-8 bg-hairline" aria-hidden="true"></li>
            <li class="flex items-center gap-2 text-bone"><span class="grid h-6 w-6 place-items-center rounded-full bg-volt text-bone">2</span> Shipping</li>
            <li class="h-px w-8 bg-hairline" aria-hidden="true"></li>
            <li class="flex items-center gap-2 text-bone"><span class="grid h-6 w-6 place-items-center rounded-full bg-volt text-bone">3</span> Payment</li>
        </ol>
            ...
            <form method="POST" action="{{ route('checkout.store') }}" data-loading-form class="grid gap-0">
                ...
                <details open class="py-6">
                    ...
                    <div class="mt-6 grid gap-5">
                        ...
                        @if ($errors->any())
                            <div class="border border-signal-bad/30 bg-black p-3 text-sm text-signal-bad">{{ $errors->first() }}</div>
                        @endif

                        <button type="submit" data-loading-label="Redirecting to Khalti…" class="motion-invert inline-flex w-fit items-center justify-center gap-3 border border-volt bg-volt px-6 py-3 text-sm font-medium uppercase tracking-wide text-bone hover:bg-bone hover:text-black">
